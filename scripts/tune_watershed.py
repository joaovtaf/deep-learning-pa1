...
def cache_predictions(model, task, dataset, device):
    """Roda a rede uma vez so e guarda probabilidade e distancia, pra varrer em cima."""
    cached = []
...
def main():
    ...
    ap.add_argument("--foreground", type=float, nargs="+", default=[0.4, 0.5, 0.6])
    ap.add_argument("--min-size", type=int, nargs="+", default=[10, 20, 40])
